...
public class ComparisonSnippetExperiment {
    ...
    public static void main(String args[]) throws Exception {
        File confFile = new File(WikiMinerTest.CONFIG_PATH) ;
        ...
        Vector<ConnectionSnippet> dataset = new Vector<>() ;

        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Kucing", tp), wikipedia.getMostLikelyArticle("Anjing", tp)) ;
//        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Kiwi", tp), wikipedia.getMostLikelyArticle("Kapiti Island", tp)) ;
        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Buku", tp), wikipedia.getMostLikelyArticle("Perpustakaan", tp)) ;
        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Jimmy Wales", tp), wikipedia.getMostLikelyArticle("Wikipedia", tp)) ;
        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Selandia Baru", tp), wikipedia.getMostLikelyArticle("Maori", tp)) ;
//        cse.addToDataset(dataset, wikipedia.getMostLikelyArticle("Waikato", tp), wikipedia.getMostLikelyArticle("Maori land wars", tp)) ;
        ...
        csw.train(dataset) ;
        csw.saveTrainingData(new File("./comparisonWorkbench/snippetWeighting_en.arff")) ;
